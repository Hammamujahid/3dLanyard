...
function Band({ maxSpeed = 50, minSpeed = 10 }) {
  ...
  const [curve] = useState(
    () =>
      new THREE.CatmullRomCurve3([ // 4 titik kurva
        new THREE.Vector3(),
        new THREE.Vector3(),
        new THREE.Vector3(),
        new THREE.Vector3(),
  ...
  // Atur wrapping dan offset tekstur kartu
  ...
  useFrame((state, delta) => {
    if (dragged) {
      ...
      vec.add(dir.multiplyScalar(state.camera.position.length() * 1.2)); // lebih responsif di mobile
